function pim = imPreProcess(im)
%IMPREPRPCESS Globally normalized an image

k = fspecial('gaussian', 46, 8.17);
pim = zeros(size(im));

for channelInd = 1:size(im, 3)
  dim = double(im(:,:,channelInd));
  ...
  lmn = conv2(dim,k,'same');
  lmnsq = conv2(dim.^2,k,'same');
  lvar = lmnsq - lmn.^2;
